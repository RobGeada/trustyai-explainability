...
ODHPROJECT=redhat-ods-applications
...
function setup_test() {
  ...
  os::cmd::expect_success "oc apply -f ${RESOURCEDIR}/common/trustyai.yaml"
}
...
function check_mm_resources() {
  ...
  os::cmd::try_until_text "curl -k https://$INFER_ROUTE/infer -d @${RESOURCEDIR}/ci/data.json" "model_name"
}
...
function check_communication(){
    ...
    os::cmd::expect_success_and_text "curl -k https://$INFER_ROUTE/infer -d @${RESOURCEDIR}/ci/data.json" "model_name"
    oc project ${ODHPROJECT}
